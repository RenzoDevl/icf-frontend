...
const props = defineProps<{
  // formato antigo (continua aceito para retrocompatibilidade)
  item?: {
    titulo: string
    tipo: string
    bairro: string
  ...
  preco?: number | string
  capa?: string
  tag?: string     // ignorado visualmente
  href?: string
  kpis?: Kpi[]
}>()
...
// Fallback de imagem caso a URL quebre
const fallback =
...
<template>
  <article
    class="group h-full flex flex-col bg-white rounded-2xl border border-slate-200/80 overflow-hidden
           transition-all duration-300
           hover:border-blue-500 hover:shadow-[0_12px_30px_rgba(37,99,235,.18)]"
  >
    ...
    <div class="flex-1 p-4 md:p-5">
      <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-600 text-sm">
        <span class="inline-flex items-center gap-1.5">
         <Icon name="ph:house-line" class="text-slate-400" />
          {{ norm.tipo }}
        </span>
        <span class="inline-flex items-center gap-1.5">
          <Icon name="ph:map-pin" class="text-slate-400" />
          {{ norm.local }}
        </span>
      </div>
      ...
      <p class="mt-2 text-[1.25rem] md:text-[1.35rem] font-extrabold text-blue-600">
        {{ brl(norm.preco) }}
      </p>
      ...
      >
        <li v-for="(k, i) in norm.kpis" :key="i" class="inline-flex items-center gap-1.5">
          <Icon v-if="(k as any).icon" :name="(k as any).icon" class="text-slate-400" />
    ...
    <div class="p-4 md:p-5 pt-0">
      <UiButton :href="norm.href" variant="outline" class="w-full">
        Ver Detalhes
      </UiButton>
    </div>
  </article>
